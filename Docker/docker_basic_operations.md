...
- Dockerfileによりコンテナの仕様を管理できる。それにより、バージョン管理出来る、共有が簡単、変更を加えやすい等のメリットを享受できる。
...
`docker stop` や `docker start` 、 `docker logs` のように特定のコンテナを対象にするコマンドは引数に**コンテナ名**か**コンテナID**を取る。
